...
public class Util implements Opcodes {
	...
	public static void invokeInterface(MethodVisitor mv, Method method) {
	...
	public static void cast(MethodVisitor mv, Class from, Class to) {
		if (to.isPrimitive())
			to = org.hashlang.jvm.Util.getBoxedClass(to);
	...
	public static void box(MethodVisitor mv, Class c) {
		if (!c.isPrimitive())
			return;
		Class boxedClass = org.hashlang.jvm.Util.getBoxedClass(c);
		try {
